...
gradient_cum_cc <- gradient_cc |> 
  select(
    site, 
    cumulative_dams, 
    `Δlc-excess/Δavg_dam_length_6-23 (ppb/m)`, 
  ...
  mutate(
    date = case_when(
      date == "Δlc-excess/Δavg_dam_length_6-23 (ppb/m)" ~ "2026-06-23",
      date == "Δlc-excess/Δcum_dam_length_7-7 (ppb/m)"  ~ "2026-07-07"
    ),
    gradient = as.numeric(gradient),
    cumulative_dams = as.numeric(cumulative_dams),
    # Adjust site factor levels if they differ from the previous dataset
    site = factor(site) 
  ) |> 
  filter(!is.na(gradient))
