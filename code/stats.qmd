...
river_reach <- compiled_isotope |>
  filter(!site %in% c("utc_isco", "tc_isco")) |>
  mutate(site = factor(site, levels = c(
    "tc_us", "tc_mid", "tc_ds", "tc_1", "tc_15", "tc_2", "tc_3",
    "coal_1", "coal_2", "coal_3", "coal_4", "coal_5", "coal_6",
    "italian_1", "italian_2", "italian_3", "italian_4", "italian_5"
  ))) |>
  arrange(site)
...
table7 <- lc_stat |>
  ...
  gt() |>
  tab_header(
    title = "Table 7. Descriptive statistics of lc-excess by sampling location"
  ) |>
...
test_results <- bind_rows(
  tidy(t.test(lc_excess ~ period, data = discharge_isco)) %>%
...
table5a <- period_summary %>%
  ...
  tab_header(title = "Table 5a. lc-excess summary statistics by flow period",
             subtitle = "Upstream and Downstream Trail Creek ISCO (peak flow: 5/14/2026)") %>%
...
table7 <- lc_stat |>
  rename(Location = location, N = n, `Mean (‰)` = mean_lc, `SD (‰)` = sd_lc, `Median (‰)` = median_lc) |>
  gt() |>
  tab_header(
    title = "Table 7. Descriptive statistics of lc-excess by sampling location") |>
  cols_align(align = "center", columns = everything()) |>
...
q_paired <- sontek %>%
  group_by(creek, date) %>%
  filter(site_num == min(site_num) | site_num == max(site_num)) %>%
  mutate(position = ifelse(site_num == min(site_num), "upstream", "downstream")) %>%
  ungroup() %>%
  select(date, creek, position, q_cfs) %>%
  distinct(date, creek, position, .keep_all = TRUE) %>%  # in case of ties
  pivot_wider(names_from = position, values_from = q_cfs) %>%
  mutate(
    diff_cfs = downstream - upstream,
    pct_change = (downstream - upstream) / upstream * 100
  ) %>%
...
table8 <- q_paired |> 
  arrange(creek, date) |> 
  gt() |> 
  fmt_number(columns = c(upstream, downstream, diff_cfs, pct_change), decimals = 3) |> 
  cols_label(
    date = "Date",
    creek = "Creek",
    upstream = "Upstream Q (cfs)",
    downstream = "Downstream Q (cfs)",
    diff_cfs = "Difference (cfs)",
    pct_change = "% Change"
  ) |> 
  tab_header(title = "Table 8. Paired discharge comparison, upstream vs. downstream sites by creek")
...
gtsave(table7, "tables/table7_statistics_river_reach.docx")
gtsave(table8, "tables/table8_discharge_cc_it.docx")
